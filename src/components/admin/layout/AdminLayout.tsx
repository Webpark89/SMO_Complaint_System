    ...
    <div className="h-screen overflow-hidden bg-[var(--background)] font-sans text-[#111827]">
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-[var(--border)] bg-white/95 px-6 backdrop-blur">
        ...
        <div className="flex items-center gap-4">
          ...
          {notificationsOpen ? (
            <div
              className="absolute right-6 top-16 w-[360px] rounded-2xl border border-[var(--border)] bg-white p-3 shadow-elegant"
              role="dialog"
            ...
            >
                ...
                {[0, 1, 2].map((i) => (
                  ...
                  >
                    <div className="text-sm font-semibold text-[#111827]">
                      เหตุการณ์การตรวจสอบใหม่
                    </div>
                    <div className="mt-1 text-xs font-medium text-slate-500">
                      {i === 0
                        ? "อัปเดตนโยบายโดยผู้ดูแลระบบ"
                        : i === 1
                          ? "มีการส่งเรื่องร้องเรียนใหม่"
                          : "เผยแพร่แบบฟอร์มสำเร็จ"}
                    </div>
                  </div>
                ))}
              </div>
              <button
                type="button"
                className="mt-3 w-full rounded-xl bg-[var(--gold)] px-4 py-2 text-sm font-semibold text-[#111827] shadow-soft transition-colors hover:opacity-95"
                onClick={() => {
